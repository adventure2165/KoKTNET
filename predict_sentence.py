...
class NERPredictor:
    def __init__(self, pred_config):
        ...
        self.args = get_args(pred_config)
        self.device = get_device(pred_config)
        self.model = load_model(pred_config, self.args, self.device)
        self.label_lst = get_labels(self.args)
    ...
    def predict(self, input_line):
        ...
        input_lines.append(input_words)
        dataset = convert_input_file_to_tensor_dataset(input_lines, self.pred_config, self.args, self.tokenizer, self.pad_token_label_id)
        # Predict
        sampler = SequentialSampler(dataset)
        ...
        for i in range(preds.shape[0]):
            for j in range(preds.shape[1]):
                if all_slot_label_mask[i, j] != self.pad_token_label_id:
                    preds_list[i].append(slot_label_map[preds[i][j]])
        tuple_list = []
        for words, preds in zip(input_lines, preds_list):
            for word, pred in zip(words, preds):
                tuple_list.append((word, pred))
        return {"input" : input_line, "pred" : preds_list, "NERS" : tuple_list}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sentence_mode", default = False,type=bool, help="Select Sentence mode when you want to predict no output files")

    pred_config = parser.parse_args()
    if pred_config.sentence_mode == True:
        pred = NERPredictor(pred_config)
        print(pred.predict())
    else:
        predict(pred_config)
